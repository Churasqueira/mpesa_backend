...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Ajuste para seu frontend quando for produção
...
BASE_URL = os.getenv("MPESA_BASE_URL", "https://api.sandbox.vm.co.mz")  # sandbox ou produção
...
def generate_token():
    """
    Gera token OAuth2 da Vodacom M-Pesa.
    """
...
async def c2b_payment(request: Request):
    """
    Inicia pagamento C2B real.
    ⚠️ USSD será enviado para o celular do usuário.
    """
    try:
        ...
        # Aqui você apenas retorna a resposta inicial do M-Pesa.
        # Usuário precisará digitar o PIN no celular (USSD).
...
async def b2c_payment(request: Request):
    """
    Envia pagamento B2C (Plataforma -> Usuário).
    """
    try:
        data = await request.json()
        msisdn = data.get("msisdn")
